...
static bool __resolve_ext_adv_report(le_ext_adv_report_evt_t *evt, u16 len)
{
    ...
    u16 event_type  = evt->Event_Type.event_type;
    u8 address_type = evt->Address_Type;
    u8 *adv_address = &evt->Address;
    u8 *data = &evt->Data;
    u8 data_length = evt-> Data_Length;
    s8 rssi = evt->RSSI;
    ...
    for (i = 0; i < evt->Data_Length;) {
        ...
        case HCI_EIR_DATATYPE_SHORTENED_LOCAL_NAME:
            ...
            log_info("ble remoter_name: %s,rssi:%d", adv_data_pt, (s8)evt->RSSI);
