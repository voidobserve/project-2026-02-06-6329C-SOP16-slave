...
static int uart_update_wait_rev_data(u16 timeout)
{
    u32 err;
...
int uart_update_api_write_then_read(u8 *buf, u8 length, u16 timeout)
{
    int ret = FALSE;
...
static void update_process_run(void)
{
    ...
    while (1) {
        if (OS_NO_ERR != os_sem_pend(&__this->rx_sem, 800)) {
            ...
            extern void usb_host_test();
            usb_host_test();
            // uart_update_send_update_ready("storage/udisk/C/update.ufw");
            continue;
        }
