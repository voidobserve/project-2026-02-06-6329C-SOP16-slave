...
#define SC_PFB_BUFF_SIZE_MIN    (LCD_SCREEN_WIDTH * 1 * (LCD_DMA_ISR_EN + 1))                   // 最小需要的内存大小
#define SC_PFB_BUFF_SIZE_MAX    (LCD_SCREEN_WIDTH * LCD_SCREEN_HEIGHT * (LCD_DMA_ISR_EN + 1))   // 最大需要的内存大小
#define SC_PFB_BUFF_SIZE_DEF    (2048)
#define SC_PFB_BUFF_SIZE_USR    (320 * 48 * 2)

#define SC_PFB_BUF_SIZE     SC_PFB_BUFF_SIZE_USR

#define  LCD_DMA_WAP         1  //软件交换高低位


#if SC_PFB_BUF_SIZE < (LCD_SCREEN_WIDTH * (LCD_DMA_ISR_EN + 1))
#error "缓冲池最少是一行像素数量，如果使用双缓存则需要x2"
#endif
