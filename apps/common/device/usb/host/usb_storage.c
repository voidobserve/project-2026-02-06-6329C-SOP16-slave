...
static bool usb_stor_online(const struct dev_node *node)
{
    ...
    if (disk) {
        log_debug("disk->dev_status:%d\n", disk->dev_status);
        return true;
        // return disk->media_sta_cur ? true : false;
    }
    return false;
}
...
void usb_host_test()
{
    ...
    struct imount *mnt = mount("udisk0", "storage/udisk0", "fat", 3, NULL);
    log_info("udisk0 mount %s\n", mnt ? "success" : "failed");
    struct vfscan *fsc = fscan("storage/", NULL, 4);
    if (fsc) {
        log_info("fscan success\n");
    } else {
        log_info("fscan failed\n");
    }
    return;
#if 0
___usb_test:
    log_info("udisk0 is %s\n", dev_online("udisk0") ? "online" : "offline");
    usb_host_mount(0, 3, 20, 200);
    log_info("udisk0 is %s\n", dev_online("udisk0") ? "online" : "offline");
    ...
    log_info("%s---%d\n", __func__, __LINE__);
    log_info("udisk0 is %s\n", dev_online("udisk0") ? "online" : "offline");
    FILE *file = fopen("storage/udisk0/C/update.ufw", "r");
    if (file) {
        log_info("open update.ufw success\n");
    } else {
        log_info("open update.ufw failed\n");
    }
    u32 lba = 0;
    while (1) {
