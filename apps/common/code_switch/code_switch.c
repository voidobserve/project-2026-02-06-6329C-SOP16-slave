...
static void code_switch_event_to_usr(u8 event, s8 sw_val)
{
    ...
    e.arg                 = DEVICE_EVENT_FROM_CODE_SWITCH;
...
static bool get_code_phase(u8 *code_data)
{
    ...
    //获取相位值
    code_last = code_now;
    code_now = (!gpio_read(sw_pdata->a_phase_io) ? 0x01 : 0x00) + (!gpio_read(sw_pdata->b_phase_io) ? 0x02 : 0x00);
...
u8 get_code_value(u8 *code_data)
{
    ...
    if (idx < ARRAY_SIZE(code_postive_list)) {
        ...
        if ((idx >= 0 && idx <= 2) || (move_now != move_last))
...
void code_switch_init(SW_PLATFORM_DATA *priv)
{
    ...
    gpio_set_pull_up(sw_pdata->a_phase_io, 1);
    ...
    gpio_set_pull_up(sw_pdata->b_phase_io, 1);
...
static void code_switch_event_handler(struct sys_event *event)
{
    if ((u32)event->arg == DEVICE_EVENT_FROM_CODE_SWITCH) {
        printf(">code switch event:%d, event:%d\n", event->u.codesw.event, event->u.codesw.value);
        sys_device_event_consume(&(event->u.dev));  // 消耗掉这次消息，不再向后推送
    }
}
SYS_EVENT_HANDLER(SYS_DEVICE_EVENT, code_switch_event_handler, 4);
