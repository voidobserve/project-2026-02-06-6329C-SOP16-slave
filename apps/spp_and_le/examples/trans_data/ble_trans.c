...
#define ATT_MTU_REQUEST_ENALBE     1    /*配置1,就是设备端主动发起交换*/
...
#define TEST_TRANS_NOTIFY_HANDLE     ATT_CHARACTERISTIC_FFF2_01_VALUE_HANDLE /*主动发送hanlde,为空则不测试发数*/
 ...
 */
/*************************************************************************************************/
void trans_uart_rx_to_ble(u8 *packet, u32 size)
{
    if (trans_con_handle && ble_comm_att_check_send(trans_con_handle, size) &&
        ble_gatt_server_characteristic_ccc_get(trans_con_handle, ATT_CHARACTERISTIC_FFF2_01_CLIENT_CONFIGURATION_HANDLE)) {
        ble_comm_att_send_data(trans_con_handle, ATT_CHARACTERISTIC_FFF2_01_VALUE_HANDLE, packet, size, ATT_OP_AUTO_READ_CCC);
    } else {
...
static void trans_resume_all_ccc_enable(u16 conn_handle, u8 update_request)
{
    ...
    ble_gatt_server_characteristic_ccc_set(conn_handle, ATT_CHARACTERISTIC_FFF2_01_CLIENT_CONFIGURATION_HANDLE, ATT_OP_NOTIFY);
...
static int trans_att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_FFF3_01_VALUE_HANDLE:
        log_info("-fff3_rx(%d):", buffer_size);
    ...
    case ATT_CHARACTERISTIC_FFF1_01_VALUE_HANDLE:
        ...
        log_info("-fff1_rx(%d):", buffer_size);
        // put_buf(buffer, buffer_size);
        // rx_update_file_write(buffer, buffer_size);
        // log_info("rx_write finished: %d", rx_update_file_get_size());
        ...
        if (ble_comm_att_check_send(connection_handle, buffer_size) &&
            ble_gatt_server_characteristic_ccc_get(trans_con_handle, ATT_CHARACTERISTIC_FFF2_01_CLIENT_CONFIGURATION_HANDLE)) {
            log_info("-loop send1\n");
            ble_comm_att_send_data(connection_handle, ATT_CHARACTERISTIC_FFF2_01_VALUE_HANDLE, buffer, buffer_size, ATT_OP_AUTO_READ_CCC);
        }
#endif
        break;
